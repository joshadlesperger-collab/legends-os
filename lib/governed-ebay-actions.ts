...
export async function createGovernedTitleExecution(listingId:string,operatorId:string){
  ...
  const decision=await prisma.operatorDecision.create({data:{listingId,operatorId,recommendedAction:"OPTIMIZE_TITLE",doctrineVersion:TITLE_INSPECTION_RULE_VERSION,decision:"follow_recommendation",beforeState:asJson(proposal.before),evidenceSnapshot:asJson({source:"title-inspection-agent",inspection,governedProposal:proposal,capturedAt:new Date().toISOString()}),observationWindowDays:30}});
